...
def run_ppmi_lsa_pipeline(count_df, k):
    ...
    lsa_df = vsm.lsa(ppmi_df, k)
    return full_word_similarity_evaluation(lsa_df)
...
def run_small_glove_evals():
    ...
    for i in [10, 100, 200]:
        ...
        giga20 = pd.read_csv(os.path.join(VSM_HOME, 'giga_window20-flat.csv.gz'), index_col=0)
        X = glv.fit(giga20)
        data[i] = full_word_similarity_evaluation(pd.DataFrame(X, index=giga20.index))['Macro-average']
        glv.sess.close()
    return data
...
def dice(u, v):
    ...
    ##### YOUR CODE HERE
    return 1 - (2 * np.sum(np.minimum(u, v)) / np.sum(np.add(u, v)))
...
def ttest(df):
    ...
    total = df.sum().sum()
    pij = df / total
    pix = df.sum(axis=1) / total
    pxj = df.sum(axis=0) / total
    mul = np.outer(pix, pxj)
    return (pij - mul) / np.sqrt(mul)
...
def subword_enrichment(df, n=4):
    ...
    ##### YOUR CODE HERE
    reps = np.stack(df.index.map(lambda w: vsm.character_level_rep(w, ngram_df, n)))
...
# The model operates on the IMDB window size 5 count matrix.
# It first applies positive PMI to the count matrix using `vsm.pmi`.
# Next, the t-test reweighting function is applied.
# At this point I originally tried using subword_enrichment on the dataframe,
# but it performed slightly worse than simply omitting the function for the imdb5 dataset.
# Next, L2 normalization is applied by using `vsm.length_norm` to prevent severe overfitting.
# The TorchAutoencoder was then used to fit the model with hyperparameters
# found with a standard grid search.
# Finally, the full_word_similarity_evaluation is applied to the dataframe
# to produce the Spearman coefficient for each evaluation dataset.
