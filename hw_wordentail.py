...
# The model subclasses from the TorchShallowNeuralClassifier and expands on the original implementation for TorchDeepNeuralClassifier.
# The layers for the sequential module are implemented as follows:
#   1. Linear layer with dimensions: input, hidden
#   2. Result applied to sigmoid activation function
#   3. A second Linear layer with dimensions: hidden, hidden
#   4. Result applied to sigmoid activation function
#   5. Dropout to randomly zero some of the elements of the tensor with probability `dropout_prob` using samples from a Bernoulli distribution
#   6. Result applied to tanh activation function
#   7. Final Linear layer with dimensions: hidden, n_classes
#
# For the `vector_func`, the glove 100D file is used rather than the original 50D.
# For the `vector_combo_func` to combine the premise and hypothesis I tried using average, sum, max, difference, though in the end concatenation still yielded the best results.
# 
# macro-F1
...
if 'IS_GRADESCOPE_ENV' not in os.environ:
    ...
    class OriginalSystemClassifier(TorchShallowNeuralClassifier):
        def __init__(self, dropout_prob=0.7, **kwargs):
            self.dropout_prob = dropout_prob
            self.glove_dim = 100
        ...
        def glove_vec_dim(self, w):
            # Return `w`'s GloVe representation if available, else return a random vector
            return self.glove_dict.get(w, randvec(w, n=self.glove_dim))

    def vec_average(u, v):
        return np.average((u, v), axis=0)
    
    def vec_sum(u, v):
        return u + v
    

    # Set model and run experiment
    model = OriginalSystemClassifier()

    nli.wordentail_experiment(
        train_data=wordentail_data['word_disjoint']['train'],
        assess_data=wordentail_data['word_disjoint']['dev'], 
        model=model, 
        vector_func=model.glove_vec_dim,
        vector_combo_func=vec_concatenate)
